...
def test_check_webhook_timeouts_outer_exception(app):
    with (
        patch("hookwise.tasks.WebhookConfig.query") as mock_query,
...
def test_check_webhook_timeouts_inner_exception(app):
    ...
    with (
        ...
        assert any("Error processing timeout for endpoint 'Test Endpoint': Inner error" in msg for msg in error_calls)
