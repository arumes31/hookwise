...
def login() -> Any:
    """Handle user login and 2FA verification."""
...
def _register() -> None:
    """Register authentication routes on the main blueprint."""
    from .routes import main_bp

    main_bp.add_url_rule("/login", endpoint="login", view_func=login, methods=["GET", "POST"])
    main_bp.add_url_rule(
        "/settings/2fa/setup", endpoint="setup_2fa", view_func=setup_2fa, methods=["GET", "POST"]
    )
    main_bp.add_url_rule("/settings/2fa/disable", endpoint="disable_2fa", view_func=disable_2fa, methods=["POST"])
    main_bp.add_url_rule("/logout", endpoint="logout", view_func=logout)
