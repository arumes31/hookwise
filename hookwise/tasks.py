...
COMPANIES_CACHE_KEY = "hookwise_companies_list"
                ...
                if not company_id and config.global_routing_enabled:
                    ...
                    if tenant_val:
                        ...
                        if not mapping:
                            ...
                            # Get all companies from ConnectWise (with caching)
                            available_companies = []
                            cached_companies = cast(Optional[bytes], redis_client.get(COMPANIES_CACHE_KEY))
                            if cached_companies:
                                available_companies = json.loads(cached_companies.decode())
                            else:
                                ...
                                if companies:
                                    ...
                                    if available_companies:
                                        redis_client.set(
                                            COMPANIES_CACHE_KEY,
                                            json.dumps(available_companies),
                                            ex=COMPANIES_CACHE_TTL,
                                        )
                                if available_companies:
