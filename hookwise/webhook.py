...
def log_webhook_rejection(config_id: str, request_id: str, error_msg: str) -> None:
...
def _verify_bearer_auth(config: WebhookConfig) -> Any:
...
def _verify_hmac_signature(config: WebhookConfig) -> Any:
    ...
    if config.hmac_secret:
        ...
        if not signature:
            return "Missing HMAC Signature", 401
...
def _verify_ip_whitelist(config: WebhookConfig) -> Any:
    """Verify client IP against whitelist if enabled."""
    if config.trusted_ips:
        ...
        for trusted_range in [ip.strip() for ip in config.trusted_ips.split(",")]:
            try:
                ...
                continue
        if not trusted:
            return f"IP {client_ip} not allowed", 403
    return None
...
def dynamic_webhook(config_id: str) -> Any:
    """Dynamic webhook ingestion endpoint."""
    ...
    if not config.is_enabled:
        ...
        log_webhook_rejection(config_id, request_id, "Endpoint is disabled")
        return jsonify({"status": "error", "message": "Endpoint is disabled"}), 403

    # Bearer Authentication
    bearer_error = _verify_bearer_auth(config)
    if bearer_error:
        msg, code = bearer_error
        log_webhook_received(status="unauthorized", config_name=config.name)
        log_webhook_rejection(config_id, request_id, msg)
        return jsonify({"status": "error", "message": msg}), code

    # HMAC Signature Verification
    hmac_error = _verify_hmac_signature(config)
    if hmac_error:
        msg, code = hmac_error
        log_webhook_rejection(config_id, request_id, msg)
        return jsonify({"status": "error", "message": msg}), code
    ...
    # IP Whitelisting
    ip_error = _verify_ip_whitelist(config)
    if ip_error:
        msg, code = ip_error
    ...
    if not data:
        ...
        log_webhook_rejection(config_id, request_id, "No JSON payload")
...
def _register() -> None:
    ...
    main_bp.add_url_rule("/w/<config_id>", view_func=dynamic_webhook, methods=["POST"])
