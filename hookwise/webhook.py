...
def _log_webhook_rejection(config_id: str, request_id: str, error_msg: str) -> None:
    """Log a rejected webhook attempt to the database."""
    try:
        try:
...
def _check_bearer_auth(config: WebhookConfig) -> Optional[Tuple[Dict[str, str], int, str]]:
    """Verify Bearer token if enabled. Returns (response_body, status_code, metric_status) if failed."""
    if not config.bearer_auth_enabled:
        return None

    auth_header = request.headers.get("Authorization")
    if not auth_header or not auth_header.startswith("Bearer "):
        return {"status": "error", "message": "Missing Bearer Token"}, 401, "unauthorized"

    token = auth_header.split(" ")[1]
    if not hmac.compare_digest(token, decrypt_string(config.bearer_token)):
        return {"status": "error", "message": "Invalid Bearer Token"}, 401, "unauthorized"

    return None


def _check_hmac_signature(config: WebhookConfig) -> Optional[Tuple[Dict[str, str], int, Optional[str]]]:
    """Verify HMAC signature if enabled. Returns (response_body, status_code, metric_status) if failed."""
    if not config.hmac_secret:
        return None

    signature = request.headers.get("X-HookWise-Signature")
    if not signature:
        return {"status": "error", "message": "Missing HMAC Signature"}, 401, None

    computed = hmac.HMAC(config.hmac_secret.encode(), request.data, hashlib.sha256).hexdigest()
    if not hmac.compare_digest(computed, signature):
        return {"status": "error", "message": "Invalid HMAC Signature"}, 401, None

    return None


def _check_ip_whitelist(config: WebhookConfig) -> Optional[Tuple[Dict[str, str], int, str]]:
    """Verify source IP against whitelist if enabled. Returns (response_body, status_code, metric_status) if failed."""
    ...
    if not trusted:
        return {"status": "error", "message": f"IP {client_ip} not allowed"}, 403, "forbidden"
...
def _register() -> None:
    ...
    def dynamic_webhook(config_id: str) -> Any:
        ...
        for check_func in [_check_bearer_auth, _check_hmac_signature, _check_ip_whitelist]:
            result = check_func(config)
            if result:
                body, code, metric_status = result
                if metric_status:
                    log_webhook_received(status=metric_status, config_name=config.name)
                _log_webhook_rejection(config_id, request_id, body["message"])
