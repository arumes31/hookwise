...
def _log_webhook_rejection(config_id: str, request_id: str, error_msg: str) -> None:
    """Log a rejected webhook request to the database."""
    try:
        try:
...
def _verify_bearer_auth(config: WebhookConfig, request_id: str) -> Optional[Tuple[Any, int]]:
    ...
    if not auth_header or not auth_header.startswith("Bearer "):
        ...
        _log_webhook_rejection(config.id, request_id, "Missing Bearer Token")
    ...
    if not hmac.compare_digest(token, decrypt_string(config.bearer_token)):
        log_webhook_received(status="unauthorized", config_name=config.name)
        _log_webhook_rejection(config.id, request_id, "Invalid Bearer Token")
        return jsonify({"status": "error", "message": "Invalid Bearer Token"}), 401

    return None


def _verify_hmac_signature(config: WebhookConfig, request_id: str) -> Optional[Tuple[Any, int]]:
    ...
    if not signature:
        _log_webhook_rejection(config.id, request_id, "Missing HMAC Signature")
    ...
    if not hmac.compare_digest(computed, signature):
        _log_webhook_rejection(config.id, request_id, "Invalid HMAC Signature")
        return jsonify({"status": "error", "message": "Invalid HMAC Signature"}), 401

    return None


def _check_ip_whitelist(config: WebhookConfig, request_id: str) -> Optional[Tuple[Any, int]]:
    ...
    if not trusted:
        ...
        _log_webhook_rejection(config.id, request_id, f"IP {client_ip} not allowed")
...
def _register() -> None:
    ...
    def dynamic_webhook(config_id: str) -> Any:
        ...
        if not config.is_enabled:
            ...
            _log_webhook_rejection(config.id, request_id, "Endpoint is disabled")
            return jsonify({"status": "error", "message": "Endpoint is disabled"}), 403

        # Authentication and Security Checks
        for check_func in [_verify_bearer_auth, _verify_hmac_signature, _check_ip_whitelist]:
            error_response = check_func(config, request_id)
            if error_response:
        ...
        if not data:
            ...
            _log_webhook_rejection(config.id, request_id, "No JSON payload")
